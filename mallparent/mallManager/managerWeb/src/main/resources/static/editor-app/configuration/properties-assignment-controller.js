...
var KisBpmChoseAssignmentCtrl = ['$scope', '$http', function($scope, $http) {
    ...
    function getDetail(value,page,limit)
    {
        ...
        $http({
            ...
            headers: {'Accept': 'application/json',
                'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'},
            url: '/user/listUser?queryText='+value+"&page="+page+"&limit="+limit+"&queryBy=4"})
            .success(function (data, status, headers, config) {
                //封装数据
                if (data != null) {
                    var accounts = [];
                    for (var i=0; i<data.length; i++) {
                        accounts.push({id:data[i].id, code : data[i].username, name : data[i].realName, index:i});
                    }
                    $scope.accounts=accounts;
                }
                refreshPage(value,page,limit,data.length);
            })
            .error(function (data, status, headers, config) {
            });
    }
