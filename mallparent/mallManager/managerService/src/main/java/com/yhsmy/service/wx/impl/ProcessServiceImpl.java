...
import com.yhsmy.entity.vo.wx.message.LocationMessage;
...
public class ProcessServiceImpl implements ProcessServiceI {
    ...
    private String getResponse(Map<String, String> requestMsgMap, String msgType, String eventType) {
        if (StringUtils.isNotBlank(msgType) && !"event".equals(msgType)) {
            ...
            if (msgType.equals(WxMessageTypeEnum.TEXT.getKey())) {
                ...
                return this.replyTextMessage(textMessage.getFromUserName(), textMessage.getToUserName(), getRobotResult(textMessage.getContent()));
                ...
                return this.replyPicMessage(picture.getFromUserName(), picture.getToUserName(), picture.getMediaId());
                ...
                // 地理位置消息，收到地址位置后响应给用户详细的地址位置，及经纬度信息
                LocationMessage locationMessage = FastJsonUtil.parseJSONObjectToObject(requestMsgMap, LocationMessage.class);
                String location = "维度= " + locationMessage.getLocation_X() + " ,经度= " + locationMessage.getLocation_Y() + " ,腾讯地图解析位置= " + getTencentMapAddress(Float.parseFloat(locationMessage.getLocation_X()), Float.parseFloat(locationMessage.getLocation_Y()));
                return this.replyTextMessage(locationMessage.getFromUserName(), locationMessage.getToUserName(), location);
            ...
            if (eventType.equals(WxEventEnum.SUBSCRIBE.getKey())) {
                // 订阅事件
                TextMessage textMessage = FastJsonUtil.parseJSONObjectToObject(requestMsgMap, TextMessage.class);
                return this.replyTextMessage(textMessage.getFromUserName(), textMessage.getToUserName(), "欢迎关注永红山盟院公众号！\n本公众号支持天气、翻译、藏头诗、笑话、歌词、手机号码归属、人工智能聊天等功能。\n使用方式：在公众号中回复关键字即可。");
    ...
    private String getTencentMapAddress(float lat, float lng) {
        String addrUrl = wechatUrlUtil.getTencentMapByLatAndLng(lat, lng), errorMeg = "未解析到相关数据";
        String tLocation = restTemplate.getForObject(addrUrl, String.class);
        try {
            JSONObject parse = JSON.parseObject(tLocation);
            if (parse.getInteger("status") == 0) {
                JSONObject result = JSON.parseObject(parse.getString("result"));
                String address = result.getString("address");
                JSONObject fAddr = result.getJSONObject("formatted_addresses");
                String recommend = fAddr.getString("recommend"); // 经过腾讯地图优化过的描述方式，更具人性化特点
                if (StringUtils.isNotBlank(address) && StringUtils.isNotBlank(recommend)) {
                    address += "(" + recommend + ")";
                }
                return address;
            }
        } catch (Exception e) {
            log.error("解析腾讯地图数据出错!", e);
        }
        return errorMeg;
    }
